...
function getAjax(page, id) {
    ...
    $("#Edit")[0].reset();
    setTimeout(function () {
        $.getJSON("api/v1/merk/" + id + "?page=" + page, function (data) {
            var no = data.from;
            var ads = data.data;
            //console.log(ads);
...
function Edit(id) {
    $.ajax({
        ...
        .done(function (data) {
            ...
            $("input[name='merk']").val(data.category);
...
function CategoryMain() {
    $.getJSON("api/v1/list-category", function (data) {
        var jumlah = data.length;

        //var child_jml = data.main2.length;
        $("#optionCategoryMain").append("<option value=''>Pilih Kategori</option>");
        $.each(data.slice(0, jumlah), function (i, data) {
            child_jml = data.main2.length;
            $("#optionCategoryMain").append("<option value=" + data.id + ">-" + data.category + "</option>");
            $.each(data.main2.slice(0, child_jml), function (i, data_main) {

                $("#optionCategoryMain").append("<option value=" + data_main.id + ">&nbsp&nbsp&nbsp" + data_main.category + "</option>");
            })
        });
